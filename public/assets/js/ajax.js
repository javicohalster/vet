...
$( "#add_paciente" ).click(function(event){
        ...
        var ci = $("#rut_add").val();
        if(!isValidCI(ci)){
            $.notify({icon: "add_alert", message: "Error en cédula de identidad verifíquela por favor."},{type: 'warning', timer: 1000});
            return;
        }
...
function isValidCI(ci) {
	...
	if(isNumeric) {	
		...
		if(ci.toString().length != 10) { 
            $.notify({icon: "add_alert", message: "La cédula debe ser de: 10 dígitos."},{type: 'warning', timer: 1000});
		...
		if (parseInt(ci, 10) == 0) { 
            $.notify({icon: "add_alert", message: "La c\u00E9dula ingresada no puede ser cero."},{type: 'warning', timer: 1000});
...
//$("#add_paciente").prop("disabled", true);
		...
		if(total != parseInt(individual)) { 
            $.notify({icon: "add_alert", message: "La c\u00E9dula ingresada no es v\u00E1lida."},{type: 'warning', timer: 1000});
    ...
    $.notify({icon: "add_alert", message: "El dato solo puede contener numeros."},{type: 'warning', timer: 1000});
